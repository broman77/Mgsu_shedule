...
class PdfScheduleParser(private val profile: UserProfile) {
    ...
    private fun parseLessonPages(pages: List<SourcePage>, url: String, label: String): List<ScheduleEvent> {
        ...
        for (p in pages) {
            ...
            val timeRows = p.frags.mapNotNull { f -> timeRegex.find(f.text)?.let { f.y to it } }
                .distinctBy { (it.first / 3).toInt() }.sortedBy { it.first }
    ...
    private fun parseExamPages(pages: List<SourcePage>, url: String, label: String): List<ScheduleEvent> {
        ...
        for (p in pages) {
            ...
            if (columns.isEmpty()) continue
            val selected = selectedColumns(columns)
            val dateRows = p.frags.mapNotNull { f -> dateRegex.find(f.text)?.let { f.y to it.value } }
                .distinctBy { (it.first / 3).toInt() }.sortedBy { it.first }
    ...
    private fun findDayMarkers(p: SourcePage): Map<Int, Float> {
        val direct = mutableMapOf<Int, Float>()
        p.frags.filter { it.x1 < p.width * .22f }.forEach { f ->
            val n = norm(f.text).replace(" ", "")
            weekdays.forEach { (name, day) -> if (n.contains(name)) direct[day] = f.y }
        }
        if (direct.size >= 2) return direct

        // Some MGSU PDFs print weekday letters vertically. Reconstruct them from the far-left zone.
        val letters = p.glyphs.filter { it.x < p.width * .12f && it.t.any(Char::isLetter) }.sortedBy { it.y }
        weekdays.forEach { (name, day) ->
            ...
            for (i in letters.indices) {
                ...
                while (j < letters.size && k < chars.size && j < i + chars.size * 3) {
                    val ch = norm(letters[j].t).firstOrNull()
                    if (ch == chars[k]) { ys += letters[j].y; k++ }
                    j++
                }
                if (k == chars.size && ys.isNotEmpty()) { direct[day] = ys.average().toFloat(); break }
            }
        }
        return direct
    }
    ...
    private fun dayForY(markers: Map<Int, Float>, y: Float): Int? {
        ...
        val sorted = markers.entries.sortedBy { it.value }
        var current = sorted.first().key
        for (e in sorted) if (y >= e.value - 35) current = e.key else break
        return current
    }
    ...
    private fun toEvent(
        ...
        val teacher = findTeacher(cleaned)
        val weeks = weekListRegex.find(cleaned)?.groupValues?.getOrNull(1)
            ?.split(',', ';')?.mapNotNull { it.trim().toIntOrNull() }.orEmpty()
        val parity = when {
            cleaned.contains("нечет", true) || cleaned.contains("нечёт", true) -> "ODD"
            cleaned.contains("четн", true) || cleaned.contains("чётн", true) -> "EVEN"
            else -> "ANY"
        }
        val title = cleaned
        ...
        val raw = "$realTitle|$group|$weekday|$exactDate|$start|$url"
    ...
    private fun matchesTeacher(cell: String, teacher: String): Boolean {
        val normalizedTeacher = norm(teacher)
        val parts = normalizedTeacher.split(Regex("[ .]+"))
            .filter { it.isNotBlank() }
        val surname = parts.firstOrNull { it.length >= 3 } ?: return false
        val initials = parts.filter { it.length == 1 && it[0].isLetter() }.take(2)

        val compactCell = norm(cell).replace(" ", "").replace(".", "")
        val compactSurname = surname.replace("-", "")
        if (!compactCell.contains(compactSurname)) return false
        if (initials.isEmpty()) return true

        val forward = compactSurname + initials.joinToString("")
        val reverse = initials.joinToString("") + compactSurname
        if (compactCell.contains(forward) || compactCell.contains(reverse)) return true

        // PDF text extraction can insert other role markers between surname and initials.
        // Requiring the surname plus both initials is still specific enough for a timetable cell.
        return initials.all { compactCell.contains(it) }
    }

    private fun findTeacher(text: String): String {
        return teacherSurnameFirstRegex.find(text)?.value
            ?: teacherInitialsFirstRegex.find(text)?.value
            ?: ""
    }
