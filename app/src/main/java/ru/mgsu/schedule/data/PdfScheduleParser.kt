...
class PdfScheduleParser(private val profile: UserProfile) {
    ...
    private fun toEvent(
        ...
        val teacher = teacherRegex.find(cleaned)?.value.orEmpty()
    ...
    private fun matchesTeacher(cell: String, teacher: String): Boolean {
        val q = norm(teacher).replace(" ", "")
        if (q.length < 3) return false
        return norm(cell).replace(" ", "").contains(q)
    }
    ...
    companion object {
        ...
        private val teacherRegex = Regex("(?i)(?:доц\\.|проф\\.|ст\\.пр\\.|пр\\.|асс\\.|зав\\.каф\\.)?\\s*[А-ЯЁ][а-яё-]+\\s+[А-ЯЁ]\\.[А-ЯЁ]\\.")
