...
class AppStore(private val context: Context) {
    ...
    fun readCache(): CachedSchedule = runCatching {
        if (!cacheFile.exists()) CachedSchedule(parserVersion = CURRENT_PARSER_VERSION)
        else {
            val cached = json.decodeFromString<CachedSchedule>(cacheFile.readText())
            if (cached.parserVersion < CURRENT_PARSER_VERSION) {
                CachedSchedule(parserVersion = CURRENT_PARSER_VERSION, lastError = "Кэш старого парсера сброшен. Обновите расписание.")
            } else cached
        }
    }.getOrDefault(CachedSchedule(parserVersion = CURRENT_PARSER_VERSION))
    ...
    private fun decodeProfileCollection(prefs: Preferences): ProfileCollection {
        ...
        if (stored != null && stored.profiles.isNotEmpty()) {
            val valid = stored.profiles.filter { validProfile(it.profile) }
            if (valid.isNotEmpty()) {
                val current = stored.currentId.takeIf { id -> valid.any { it.id == id } } ?: valid.first().id
                return stored.copy(currentId = current, profiles = valid)
            }
        }
        val legacy = prefs[profileKey]?.let { runCatching { json.decodeFromString<UserProfile>(it) }.getOrNull() }
        return if (legacy != null && validProfile(legacy)) {
            val normalized = normalizeProfile(legacy)
            val entry = SavedProfile("legacy-profile", profileDisplayName(normalized), normalized, 0L)
