...
object ScheduleParsingRules {
    ...
    // Actual name tokens are case-sensitive. This deliberately rejects fragments such as
    // "в-молоткова" that the old permissive regexp could turn into a fake teacher profile.
    private val surnameFirstRegex = Regex(
        "(?:доц\\.|проф\\.|ст\\.\\s*пр\\.|ст\\.пр\\.|преп\\.|асс\\.|зав\\.\\s*каф\\.|зав\\.каф\\.)?\\s*" +
            "([А-ЯЁ][а-яё]{2,}(?:-[А-ЯЁ][а-яё]{2,})?)\\s+([А-ЯЁ])\\.?\\s*([А-ЯЁ])\\.?"
    )
    private val initialsFirstRegex = Regex(
        "(?:доц\\.|проф\\.|ст\\.\\s*пр\\.|ст\\.пр\\.|преп\\.|асс\\.|зав\\.\\s*каф\\.|зав\\.каф\\.)?\\s*" +
            "([А-ЯЁ])\\.?\\s*([А-ЯЁ])\\.?\\s+([А-ЯЁ][а-яё]{2,}(?:-[А-ЯЁ][а-яё]{2,})?)"
    )
    ...
    fun canonicalTeacher(raw: String): String? {
        ...
        initialsFirstRegex.find(s)?.let { m ->
            return "${m.groupValues[3]} ${m.groupValues[1]}.${m.groupValues[2]}."
        }
        return null
    }
